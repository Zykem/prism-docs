...
-- Time control mode:
--   'managed'  = Climatime controls time (default, current behavior)
--   'realtime' = In-game time syncs to real server wall clock
--   'disabled' = Climatime does not manage time (let other resources handle it)
Config.TimeMode = 'managed'

-- Timezone offset (hours) for realtime mode. e.g. -5 for EST, 1 for CET, 0 for server local time
...
-- What happens to the active schedule when admin manually changes weather?
--   'stop'      = schedule auto-deactivates, admin must re-enable (recommended)
--   'temporary' = schedule stays active, next cycle tick picks new weather automatically
Config.ScheduleOverrideBehavior = 'stop'
